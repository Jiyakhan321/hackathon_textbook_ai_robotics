...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://hackathon-textbook-ai-robotics.vercel.app",  # Production Vercel URL
        "http://localhost:3000",  # Local development
        "http://localhost:3001",  # Alternative local dev
        "http://localhost:8080",  # Docusaurus local
        "http://localhost:3002",  # Alternative local dev
    ],
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.getenv("PORT", 7860))  # Use port 7860 as specified
    uvicorn.run(
        "main:app",
        host="0.0.0.0",
        port=port,
